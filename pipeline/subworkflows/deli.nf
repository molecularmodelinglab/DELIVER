...
process DecodeChunk {
    ...
    output:
    path "${prefix}_${fastq_chunk.name}_decoded.tsv", emit: decoded_tsv
    path "${prefix}_${fastq_chunk.name}_decode_statistics.json", emit: decode_stats
    path "${prefix}_${fastq_chunk.name}_deli.log", emit: deli_log
    path "${prefix}_${fastq_chunk.name}_failed_decoding.tsv", emit: failed_tsv, optional: true
    ...
    deli ${deli_args} decode run \
        ...
        "${fastq_chunk}" \
        --out-dir ./ \
        --prefix "${prefix}_${fastq_chunk.name}" \
        --skip-report \
        ${fastq_info_flag} \
        ${save_failed_flag}

    mv deli.log ${prefix}_${fastq_chunk.name}_deli.log
    """

    stub:
    """
    touch ${prefix}_${fastq_chunk.name}_decoded.tsv
    echo '{}' > ${prefix}_${fastq_chunk.name}_decode_statistics.json
    touch ${prefix}_${fastq_chunk.name}_deli.log
    """
}
